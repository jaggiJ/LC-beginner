...
[[ -f passwd1 ]] && mv passwd1 backup-passwd1 \
    && echo Made backup: '"passwd1"' into '"backup-passwd1"' \
    && echo Press a key && read -r

[[ -f passwd2 ]] && mv passwd2 backup-passwd2 \
    && echo Made backup: '"passwd2"' into '"backup-passwd2"' \
    && echo Press a key && read -r
...
if [[ $(wc -c passwd1|cut -d" " -f1) -eq 1048576 ]] ; then
    echo Successfully created file: "$(du -h passwd1)"
else
